...
struct
  ...
  *)
  let rec evaluate_lcmd (prog : UP.prog) (lcmd : LCmd.t) (state : State.t) :
      State.t list =
    let aux (prog : UP.prog) lcmd state =
          ...
          | Some v_x -> (
              ...
              | _           ->
                  raise
                    (Failure
                       (Printf.sprintf
                          "ERROR: AssumeType: Cannot assume type %s for \
                           variable %s."
                          (Type.str t) x)) )
          | _        ->
              raise
              ...
              let rec expand_macro (macro : Macro.t) (args : Expr.t list) :
                ...
                if params_card <> args_card then
                  raise
                    (Failure
                       (Printf.sprintf
                          "Macro %s called with incorrect number of \
                           parameters: %d instead of %d."
    ...
    in
    L.with_normal_phase ~title:"Evaluation of logic command" (fun () ->
        aux prog lcmd state)
      ...
      | LAction (x, a, es) -> (
          ...
          | AFail errs ->
              if not (ExecMode.concrete_exec !Config.current_exec_mode) then
                ...
                | _                  ->
                    raise
                      (Fmt.failwith "Local Action Failed: %a" Cmd.pp_indexed
                         cmd)
          ...
          let sp_t, sp_f =
            ...
            | true, true   ->
                ...
                ( List.map (fun x -> (x, j)) (State.assume ~unfold:true state vt),
                  List.map
                    (fun x -> (x, k))
                    (State.assume ~unfold:true state' vf) )
          in
          let sp = sp_t @ sp_f in
...
*)
  let evaluate_proc
      ...
      let args =
        List.map
          (fun x ->
            ...
            | None     ->
                raise
                  (Failure "Symbolic State does NOT contain formal parameter"))
          params
      in
    ...
    in
    L.with_normal_phase ~title:("Execution of " ^ name) (fun () ->
        aux ret_fun prog name params state)
